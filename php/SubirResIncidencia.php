...
for ($indiceHoja = 0; $indiceHoja < $totalDeHojas; $indiceHoja++) {
    ...
    for($a = 1; $a < $totalRow; $a++){
        $arrayDatos['numcontrato'] = $hojaActual->getCell("K$nb")->getValue();//numcontrato
        $arrayDatos['observacion'] = $hojaActual->getCell("V$nb")->getValue();//observación
        $arrayDatos['fecha'] = $hojaActual->getCell("P$nb")->getValue();//observación
...
function actualizarProducto($conn,$arrayDatos){
    ...
    if($prod[0] && !compruebaEstado($prod[2])){
        ...
        //almacenaError("Incidencia: ".$prod[1]." ".$arrayDatos['numcontrato']);
    }else{
        almacenaError('O no esta o es incidencia '.$prod[1]." ".$arrayDatos['numcontrato']);
...
function updateProd($conn,$arrayDatos,$idprod){
    ...
    $sentencia=$conn->prepare("UPDATE productos SET ultimo_estado = 'incidencia', fecha_edicion = ? WHERE numcontrato = ?");
    $sentencia->bindParam(1,$arrayDatos['fecha']);
    $sentencia->bindParam(2,$arrayDatos['numcontrato']);
    $sentencia->execute();
    if($sentencia->rowCount() > 0){
        if(updateEstado($conn,$idprod,'incidencia',$arrayDatos['fecha'])){
...
function updateProdDupli($conn,$arrayDatos,$idprod){
    global $bien,$mal,$dupli;
    //echo $arrayDatos['estado'].' '.$arrayDatos['numcontrato'].' '.$arrayDatos['observacion'].' '.$idprod;
    $sentencia=$conn->prepare("UPDATE productos_duplis SET ultimo_estado = 'hecho', fecha_edicion = ? WHERE numcontrato = ?");
    $sentencia->bindParam(1,date('Y-m-d H:i:s'));
    $sentencia->bindParam(2,$arrayDatos['numcontrato']);
    $sentencia->execute();
    if($sentencia->rowCount() > 0){
        if(updateEstado($conn,$idprod,'hecho')){
           if(insertaObs($conn,$idprod,$arrayDatos['observacion'])){
                $bien = $bien + 1;

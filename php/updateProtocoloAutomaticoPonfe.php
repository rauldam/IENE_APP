...
function obtenerProtocolosAutomaticos($conn){
    ...
    $datetime = date('Y-m-d', strtotime('-15 days'));
    
    $sql = "SELECT * FROM protocoloautomatico WHERE fecha < ? AND idred = 19";
    //$s = "SELECT * FROM protocoloautomatico WHERE fecha < '$datetime' AND idred = 19";
    //echo $datetime;
    $sentencia=$conn->prepare($sql);
        ...
        if($sentencia2->rowCount() > 0){
           ...
           // print_r($datosProds);
            ...
            if($sentencia3->rowCount() > 0){
                $sentencia4 = $conn->prepare("UPDATE productos SET ultimo_estado = 'generico', fecha_edicion = '".date('Y-m-d H:i:s')."' WHERE idproductos = $idprod");
                $sentencia4->execute();
                if($sentencia4->rowCount() > 0){
                    $sentencia5 = $conn->prepare("INSERT INTO `estados`(`tipo_estado`, `fecha`, `productos_idproductos`) VALUES ('generico','".date('Y-m-d H:m:s')."',$idprod)");
                    $sentencia5->execute();
                    if($sentencia5->rowCount() > 0){
                        $sentencia6 = $conn->prepare("INSERT INTO `observaciones`(`mensaje`, `fecha`, `es_red`,`productos_idproductos`) VALUES ('<p>Se ha pasado automáticamente a generico</p>','".date('Y-m-d H:m:s')."','n',$idprod)");
                        $sentencia6->execute();

...
for ($indiceHoja = 0; $indiceHoja < $totalDeHojas; $indiceHoja++) {
    ...
    for($a = 1; $a < $totalRow; $a++){
        ...
        $arrayDatos['nombreRed'] = strtolower($hojaActual->getCell("AA$nb")->getValue()); // NOMBRE DE LA RED
        $arrayDatos['curso'] = $hojaActual->getCell("AE$nb")->getValue();//CURSO
        $arrayDatos['anyo'] = $hojaActual->getCell("AF$nb")->getValue();//PRECIO
        $arrayDatos['formaPago'] = $hojaActual->getCell("AG$nb")->getValue();//FORMA DE PAGO
...
if(unlink('../users/fases/fase.xlsx')){
    echo "Se ha eliminado el archivo FASE para evitar problemas <br>";
}
...
function insertaProd($conn,$arrayDatos,$idcli,$idemp = null,$idred = null){
    global $bien,$mal,$dupli,$staPonfe,$tecnicoPonfe;
    if($idemp != null && $idred != null){
        $arrayDatos['idempleado'] = $idemp;
        $arrayDatos['idred'] = $idred;
    }
    if(!compruebaProd($conn,$idcli,$arrayDatos['prod'],$arrayDatos['idred'],$arrayDatos['numcontrato'])){
        $sentencia=$conn->prepare("INSERT INTO productos (tipo_producto,empresa_fiscal,numcontrato,tipo_fase,fecha_creacion,fecha_edicion,ultimo_estado,precio,usuario_comercial,clientes_idclientes,empleado_idempleado,red_idred,curso,anyo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        $sentencia->bindParam(6,date('Y-m-d H:i:s'));
        if($arrayDatos['prod'] == 'explicacion'){
            $estado = 'pendiente_explicacion';
            $sentencia->bindParam(7,$estado);
        }else{
            $estado = 'pendiente';
            $sentencia->bindParam(7,$estado);
        }
        $sentencia->bindParam(8,$arrayDatos['precio']);
        $sentencia->bindParam(9,$arrayDatos['usuario']);
        $sentencia->bindParam(10,$idcli);
        if($arrayDatos['idred'] == 19 && $idemp == null){
            if($staPonfe < 6){
                $sentencia->bindParam(11,$tecnicoPonfe);
                $staPonfe++;
                if($staPonfe > 5){
                    $staPonfe = 0;
                    switch($tecnicoPonfe){
            ...
            $sentencia->bindParam(11,$arrayDatos['idempleado']);
        }
        $sentencia->bindParam(12,$arrayDatos['idred']);
        if($arrayDatos['fase'] == "estandar"){
            $sentencia->bindParam(13,$arrayDatos['curso']);
        }else{
            $nadie = "Nadie";
            $sentencia->bindParam(13,$nadie);
        }
        if(!empty($arrayDatos['anyo'])){
            $sentencia->bindParam(14,$arrayDatos['anyo']);
        }else{
            $any = date('Y');
            $sentencia->bindParam(14,$any);
        }
        ...
        if($sentencia->rowCount() > 0){
            ...
            if(insertaEstado($conn,$lastId,$arrayDatos['prod'])){
                if(insertaLlamadas($conn,$lastId)){
                    if($arrayDatos['prod'] == "registro"){
                        if(crearPlantillaRegistro($arrayDatos['cif'],$arrayDatos['nombreRed'])){
                            $bien = $bien + 1;
                            almacenaError('Subido correctamente el prod '.$arrayDatos['prod'].' del cliente '.$idcli.' BIEN: '.$bien);
                        }else{
                            $bien = $bien + 1;
                            almacenaError('Subido correctamente el prod '.$arrayDatos['prod'].' del cliente '.$idcli.', pero no se ha creado la plantilla ni el manual BIEN: '.$bien);
                        }
                    }else{
                        $bien = $bien + 1;
                        almacenaError('Subido correctamente el prod '.$arrayDatos['prod'].' del cliente '.$idcli.' BIEN: '.$bien);
                    }
                    if($arrayDatos['prod'] == "alergenos"){
                        crearSoloAlergeno($arrayDatos['cif'],$arrayDatos['nombreRed']);
                    }
                    //$mensaje = '<p>Forma de pago: '+$arrayDatos['formaPago']+'</p>';
                    //$mensaje = $mensaje+'<p>'+$arrayDatos['observacion']+'</p>';
			...
			almacenaError('Tienes un error en la fase y no se puede subir, por favor revísala bien');
	}
    }else{
        if(insertaProdDupli($conn,$arrayDatos,$idcli,$arrayDatos['idempleado'],$arrayDatos['idred'])){
            $dupli = $dupli + 1;
            almacenaError('Producto '.$arrayDatos['prod'].' totalmente duplicado para el cliente '.$idcli.' con red '.$arrayDatos['idred'].' y fase '.$arrayDatos['fase'].' con contrato '.$arrayDatos['numcontrato'].', SE HA SUBIDO A LA TABLA DE PROD DUPLICADOS, DUPLI: '.$dupli);
        }else{
            $dupli = $dupli + 1;
            almacenaError('Producto '.$arrayDatos['prod'].' totalmente duplicado para el cliente '.$idcli.' con red '.$arrayDatos['idred'].' y fase '.$arrayDatos['fase'].' con contrato '.$arrayDatos['numcontrato'].', NO SE HA SUBIDO A LA TABLA DE PROD DUPLICADOS, DUPLI: '.$dupli);
        }
    }
}

function insertaProdDupli($conn,$arrayDatos,$idcli,$idemp = null,$idred = null){
    global $bien,$mal,$dupli,$staPonfe,$tecnicoPonfe;
    $arrayDatos['idempleado'] = $idemp;
    $arrayDatos['idred'] = $idred;
    $sentencia=$conn->prepare("INSERT INTO productos_duplis (tipo_producto,empresa_fiscal,numcontrato,tipo_fase,fecha_creacion,fecha_edicion,ultimo_estado,precio,usuario_comercial,clientes_idclientes,empleado_idempleado,red_idred) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
    $sentencia->bindParam(1,$arrayDatos['prod']);
    $sentencia->bindParam(2,$arrayDatos['empresa_fiscal']);
    $sentencia->bindParam(3,$arrayDatos['numcontrato']);
    $sentencia->bindParam(4,$arrayDatos['fase']);
...
function insertaEstado($conn,$lastInsertId,$prod){
    if($prod == 'explicacion'){
        $estado = 'pendiente_explicacion';
        ...
        $sentencia->bindParam(3,$lastInsertId);
        $sentencia->execute();
        if($sentencia->rowCount() > 0){
            return true;
        }else{
...
function compruebaProd($conn,$idcliente,$prod,$red,$numcontrato){
    ...
    if($duplis == 'n'){
        ...
        $anyo = date('Y');
        if($prod == 'explicacion'){
            $sql = "SELECT * FROM productos WHERE tipo_producto = ? AND clientes_idclientes = ? AND red_idred = ? AND ultimo_estado != ? AND numcontrato = ? AND anyo = ?";
            $sentencia=$conn->prepare($sql);
            $sentencia->bindParam(1,$prod);
            $sentencia->bindParam(2,$idcliente);
            $sentencia->bindParam(3,$red);
            $sentencia->bindParam(4,$cancel);
            $sentencia->bindParam(5,$numcontrato);
            $sentencia->bindParam(6,$anyo);
            $sentencia->execute();
            if($sentencia->rowCount() > 0){
                $devolver = true;
            }else{
            ...
            $sql = "SELECT * FROM productos WHERE tipo_producto = ? AND clientes_idclientes = ? AND red_idred = ? AND ultimo_estado != ? AND anyo = ?";
            $sentencia=$conn->prepare($sql);
            $sentencia->bindParam(1,$prod);
            $sentencia->bindParam(2,$idcliente);
            $sentencia->bindParam(3,$red);
            $sentencia->bindParam(4,$cancel);
            $sentencia->bindParam(5,$anyo);
            $sentencia->execute();
            if($sentencia->rowCount() > 0){
                $devolver = true;
            }else{
...
function crearSoloAlergeno($cif,$red){
    if (!file_exists('../users/'.$cif.'/'.$red.'/')) {
        if(mkdir('../users/'.$cif.'/'.$red.'/alergenos/',0777,true)){
           return true;
        }else{
            return false;
        }
...
function crearPlantillaRegistro($cif,$red){
    if (!file_exists('../users/'.$cif.'/'.$red.'/registro/')) {
        if(mkdir('../users/'.$cif.'/'.$red.'/registro/',0777,true)){
            $sourcePlantilla = "../Plantilla.xlsx";
            $sourceManual = "../Manual.pdf";
            $destPla = "../users/".$cif."/".$red."/registro/Plantilla.xlsx";
            $destMan = "../users/".$cif."/".$red."/registro/Manual.pdf";
            if(copy($sourcePlantilla, $destPla) && copy($sourceManual, $destMan)){
                return true;
            }else{
                return false;
            }

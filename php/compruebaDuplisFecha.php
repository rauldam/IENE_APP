...
for ($indiceHoja = 0; $indiceHoja < $totalDeHojas; $indiceHoja++) {
    ...
    for($a = 1; $a < $totalRow; $a++){
        ...
        $arrayDatos['fecha_creacion'] = $hojaActual->getCell("X$nb")->getFormattedValue();//FECHA
...
function compruebaProd($conn,$idcliente,$prod,$red,$fase,$fecha){
    ...
    if($duplis == 'n'){
        $cancel = "cancelado";
        $sql = "SELECT * FROM productos WHERE tipo_producto = $prod AND clientes_idclientes = $idcliente AND red_idred = $red AND ultimo_estado != $cancel AND fecha_creacion < $fecha";

...
$rutaArchivo = "../users/fases/hechos.xlsx";
...
for ($indiceHoja = 0; $indiceHoja < $totalDeHojas; $indiceHoja++) {
    ...
    for($a = 1; $a < $totalRow; $a++){
        ...
        $arrayDatos['observacion'] = $hojaActual->getCell("K$nb")->getValue();//observación
        $arrayDatos['fecha'] = $hojaActual->getCell("E$nb")->getValue();//observación
...
function actualizarProducto($conn,$arrayDatos){
    ...
    if($prod[0] && compruebaEstado($prod[2])){
        ...
        almacenaError('O no esta o es hecho');
...
function updateProd($conn,$arrayDatos,$idprod){
    ...
    $sentencia=$conn->prepare("UPDATE productos SET ultimo_estado = 'hecho', fecha_edicion = ? WHERE numcontrato = ?");
    $sentencia->bindParam(1,$arrayDatos['fecha']);
    $sentencia->bindParam(2,$arrayDatos['numcontrato']);
    $sentencia->execute();
    if($sentencia->rowCount() > 0){
        if(updateEstado($conn,$idprod,'hecho',$arrayDatos['fecha'])){

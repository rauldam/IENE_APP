...
$opts = array(
	...
	'roots' => array(
		...
		array( 
			...
			'path'          => '../../users/'.$ruta,                 // path to files (REQUIRED)
			'URL'           => dirname($_SERVER['PHP_SELF']) . '/../../users/'.$ruta, // URL to files (REQUIRED)
			//'trashHash'     => 't1_Lw',                     // elFinder's hash of trash folder
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
			'uploadDeny'    => array('all'),                // All Mimetypes not allowed to upload
			'uploadAllow'   => array('application/pdf','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','image/png'), // Mimetype `image` and `text/plain` allowed to upload
		...
		/*array(
			'id'            => '1',
			'driver'        => 'Trash',
			'path'          => '../../users/'.$ruta.'/.trash/',
			'tmbURL'        => dirname($_SERVER['PHP_SELF']) . '/../../users/'.$ruta.'/.trash/.tmb/',
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
			'uploadDeny'    => array('all'),                // Recomend the same settings as the original volume that uses the trash
			'uploadAllow'   => array('image/x-ms-bmp', 'image/gif', 'image/jpeg', 'image/png', 'image/x-icon', 'text/plain','application/pdf','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'), // Same as above

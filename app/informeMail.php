...
<body>
                ...
                <div class="row">
                    <div class="col-12">
                        <div align="center">
                            ...
                            <small>Desde aquí podrás obtener informes detallados de los mail enviados y sin enviar solo debes crear el filtro que desees</small>
                        </div>
                        <hr>
                    </div>
                    <div align="center">
                       ...
                       <form class="form-sample" id="filtros">
                        <select class="form-control-sm" id="cliente" onchange="controlSelect(this.value)" name="cliente" required>
                            <option selected value="s">Enviados</option>
  ...
  <script>
      ...
      var url = "../php/datatables/server_processing_mail.php?clientes=s";
     // url = url+fecha;
    ...
    $('#tabla tbody').on('dblclick', 'tr', function () {
        ...
        var idmail = data['idmail'];
        console.log(idmail);
        var x = confirm('¿Desea bloquear el envío de este correo?');
        if(x){
            $.ajax({
                url:"../php/v1/bloquearEmail",
                data:{"idmail":idmail},
                type:"POST",
                ...
                success: function(res){
                    if(res){
                        alert("Se ha bloqueado correctamente el envío del email","success");
                        window.location.reload(true);
                    }else{
                        alert("No se ha bloqueado correctamente el envío del email","error");
                    }
    ...
    $('#filtros').on('submit',function(e){
        ...
        tabla.ajax.url("../php/datatables/server_processing_mail.php?clientes="+datos[0]['value']).load();
    });
  </script>
</html>
